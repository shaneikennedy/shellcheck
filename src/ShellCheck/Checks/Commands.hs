...
checkCommand map t@(T_SimpleCommand id _ (cmd:rest)) =
  fromMaybe (return ()) $ do
    name <- getLiteralString cmd
    return $
      if '/' `elem` name
        then Map.findWithDefault nullCheck (Basename $ basename name) map t
        else do
...
getChecker list =
  Checker {perScript = const $ return (), perToken = checkCommand map}
  where
  ...
  where
    ...
    f word =
      case getLiteralString word of
        ...
        Just s -- Eliminate false positives by only looking for dupes in SET2?
         -> do
          ...
          unless ("[:" `isPrefixOf` s || "[=" `isPrefixOf` s) $
            when
              ("[" `isPrefixOf` s &&
               "]" `isSuffixOf` s && (length s > 2) && ('*' `notElem` s)) $
            info
              (getId word)
              2021
...
prop_checkFindNameGlob2 =
  verify checkFindNameGlob "find / -type f -ipath *(foo)"
...
prop_checkNeedlessExpr4 =
  verifyNot checkNeedlessExpr "foo=$(expr foo \\< regex)"

checkNeedlessExpr = CommandCheck (Basename "expr") f
  where
    f t =
  ...
  where
    ...
    f cmd (x:r) =
      let str = getLiteralStringExt (const $ return "_") x
       in if str `elem` [Just "--", Just "-e", Just "--regex"]
            then checkRE cmd r -- Regex is *after* this
            else if skippable str
    ...
    wordStartingWith c = head . filter ([c] `isPrefixOf`) $ candidates
      where
        candidates =
          sampleWords ++
          map (\(x:r) -> toUpper x : r) sampleWords ++ [c : "test"]
...
prop_checkTrapQuotes3 =
  verify checkTrapQuotes "trap \"echo $((1+num))\" EXIT DEBUG"
...
returnOrExit multi invalid = (f . arguments)
  where
    f (first:second:_) = multi (getId first)
    f [value] = when (isInvalid $ literal value) $ invalid (getId value)
    f _ = return ()
    isInvalid s =
      s == "" ||
      any (not . isDigit) s ||
      length s > 5 ||
      let value = (read s :: Integer)
       in value > 255
...
prop_checkFindExecWithSingleArgument1 =
  verify checkFindExecWithSingleArgument "find . -exec 'cat {} | wc -l' \\;"

prop_checkFindExecWithSingleArgument2 =
  verify checkFindExecWithSingleArgument "find . -execdir 'cat {} | wc -l' +"

prop_checkFindExecWithSingleArgument3 =
  verifyNot checkFindExecWithSingleArgument "find . -exec wc -l {} \\;"
...
prop_checkUnusedEchoEscapes2 =
  verifyNot checkUnusedEchoEscapes "echo -e 'foi\\nbar'"

prop_checkUnusedEchoEscapes3 = verify checkUnusedEchoEscapes "echo \"n:\\t42\""

prop_checkUnusedEchoEscapes4 = verifyNot checkUnusedEchoEscapes "echo lol"

prop_checkUnusedEchoEscapes5 =
  verifyNot checkUnusedEchoEscapes "echo -n -e '\n'"
...
checkUnusedEchoEscapes = CommandCheck (Basename "echo") f
  where
    ...
    f cmd =
      ...
      unless (cmd `hasFlag` "e") $ mapM_ examine $ arguments cmd
    examine token = do
...
prop_checkInjectableFindSh1 =
  verify checkInjectableFindSh "find . -exec sh -c 'echo {}' \\;"

prop_checkInjectableFindSh2 =
  verify checkInjectableFindSh "find . -execdir bash -c 'rm \"{}\"' ';'"

prop_checkInjectableFindSh3 =
  verifyNot checkInjectableFindSh "find . -exec sh -c 'rm \"$@\"' _ {} \\;"
...
prop_checkFindActionPrecedence1 =
  verify
    checkFindActionPrecedence
    "find . -name '*.wav' -o -name '*.au' -exec rm {} +"

prop_checkFindActionPrecedence2 =
  verifyNot
    checkFindActionPrecedence
    "find . -name '*.wav' -o \\( -name '*.au' -exec rm {} + \\)"

prop_checkFindActionPrecedence3 =
  verifyNot checkFindActionPrecedence "find . -name '*.wav' -o -name '*.au'"
...
checkFindActionPrecedence = CommandCheck (Basename "find") (f . arguments)
  where
    ...
    f list
      | length list < length pattern = return ()
    f list@(_:rest) =
    ...
    isMatch =
      isParam
        ["-name", "-regex", "-iname", "-iregex", "-wholename", "-iwholename"]
    isAction =
      isParam
        [ "-exec"
        , "-execdir"
...
prop_checkMkdirDashPM4 =
  verify checkMkdirDashPM "mkdir --parents --mode=0755 a/b"
...
prop_checkMkdirDashPM14 =
  verifyNot checkMkdirDashPM "mkdir -p -m 0755 singlelevel"

prop_checkMkdirDashPM15 = verifyNot checkMkdirDashPM "mkdir -p -m 0755 ../bin"

prop_checkMkdirDashPM16 =
  verify checkMkdirDashPM "mkdir -p -m 0755 ../bin/laden"
...
prop_checkMkdirDashPM21 =
  verifyNot checkMkdirDashPM "mkdir -p -m 0755 ../../bin"
...
prop_checkNonportableSignals7 =
  verifyNot checkNonportableSignals "trap 'stop' int"
  ...
  where
    ...
    check param =
      potentially $ do
        ...
        let id = getId param
        return $
          sequence_ $ mapMaybe (\f -> f id str) [checkNumeric, checkUntrappable]
    ...
    checkUntrappable id str = do
      ...
      return $ err id 2173 "SIGKILL/SIGSTOP can not be trapped."
...
checkInteractiveSu = CommandCheck (Basename "su") f
  where
    f cmd =
      when (length (arguments cmd) <= 1) $ do
        path <- getPathM cmd
...
prop_checkPrintfVar8 =
  verifyNot checkPrintfVar "printf '%s %s %s' \"${var[@]}\""
...
checkPrintfVar = CommandCheck (Exactly "printf") (f . arguments)
  where
    f (doubledash:rest)
      | getLiteralString doubledash == Just "--" = f rest
    f (dashv:var:rest)
      | getLiteralString dashv == Just "-v" = f rest
    ...
    check format more = do
        ...
        return $
          case () of
            ...
            () ->
              ...
              show formatCount ++
              " variables, but is passed " ++ show argCount ++ " arguments."
      ...
      where
        onlyTrailingTs format argCount = all (== 'T') $ drop argCount format
...
checkUuoeCmd = CommandCheck (Exactly "echo") (f . arguments)
  where
    msg id =
      style id 2005 "Useless echo? Instead of 'echo $(cmd)', just use 'cmd'."
...
checkSetAssignment = CommandCheck (Exactly "set") (f . arguments)
  where
    f (var:value:rest) =
      let str = literal var
       in when (isVariableName str || isAssignment str) $ msg (getId var)
    f (var:_) = when (isAssignment $ literal var) $ msg (getId var)
    f _ = return ()
    msg id =
      warn id 2121 "To assign a variable, use just 'var=value', no 'set ..'."
...
prop_checkExportedExpansions4 =
  verifyNot checkExportedExpansions "export ${foo?}"

checkExportedExpansions =
  CommandCheck (Exactly "export") (mapM_ check . arguments)
  where
    check t =
      potentially $ do
        ...
        return . warn (getId t) 2163 $
          "This does not export '" ++
          name ++ "'. Remove $/${} for that, or use ${var?} to quiet."
...
prop_checkAliasesUsesArgs3 =
  verify checkAliasesUsesArgs "alias a=\"echo \\${@}\""
...
checkAliasesUsesArgs = CommandCheck (Exactly "alias") (f . arguments)
  where
    ...
    f = mapM_ checkArg
    checkArg arg =
      let string = fromJust $ getLiteralStringExt (const $ return "_") arg
...
prop_checkAliasesExpandEarly1 =
  verify checkAliasesExpandEarly "alias foo=\"echo $PWD\""

prop_checkAliasesExpandEarly2 = verifyNot checkAliasesExpandEarly "alias -p"

prop_checkAliasesExpandEarly3 =
  verifyNot checkAliasesExpandEarly "alias foo='echo {1..10}'"

checkAliasesExpandEarly = CommandCheck (Exactly "alias") (f . arguments)
  where
    f = mapM_ checkArg
    checkArg arg
      | '=' `elem` concat (oversimplify arg) =
        forM_ (take 1 $ filter (not . isLiteral) $ getWordParts arg) $ \x ->
          warn
            (getId x)
            2139
...
prop_checkFindWithoutPath4 =
  verifyNot checkFindWithoutPath "find -H -L \"$path\" -print"
...
prop_checkTimeParameters4 =
  verifyNot checkTimeParameters "command time -f lol sleep 10"
...
prop_checkTimedCommand1 =
  verify checkTimedCommand "#!/bin/sh\ntime -p foo | bar"

prop_checkTimedCommand2 =
  verify checkTimedCommand "#!/bin/dash\ntime ( foo; bar; )"
...
checkLocalScope =
  CommandCheck (Exactly "local") $ \t ->
    whenShell [Bash, Dash] $ -- Ksh allows it, Sh doesn't support local
     do
      path <- getPathM t
...
prop_checkDeprecatedTempfile2 =
  verifyNot checkDeprecatedTempfile "tempfile=$(mktemp)"

checkDeprecatedTempfile =
  CommandCheck (Basename "tempfile") $ \t ->
    warn
      (getId $ getCommandTokenOrThis t)
      2186
  ...
  where
    ...
    f t@(T_SimpleCommand _ _ (cmd:arg1:_)) = do
      ...
      potentially $ do
        ...
        return $
          check (getId arg1) (map (: []) $ filter (/= ':') options) caseCmd
    f _ = return ()
    ...
    warnUnhandled optId caseId str =
      warn caseId 2213 $
      "getopts specified -" ++ str ++ ", but it's not handled by this 'case'."
    warnRedundant (key, expr) =
      potentially $ do
        str <- key
        guard $ str `notElem` ["*", ":", "?"]
        return $ warn (getId expr) 2214 "This case is not specified by getopts."
    getHandledStrings (_, globs, _) = map (\x -> (literal x, x)) globs
...
prop_checkCatastrophicRm3 =
  verifyNot checkCatastrophicRm "rm -r /home/${USER:?}/*"

prop_checkCatastrophicRm4 =
  verify checkCatastrophicRm "rm -fr /home/$(whoami)/*"

prop_checkCatastrophicRm5 =
  verifyNot checkCatastrophicRm "rm -r /home/${USER:-thing}/*"

prop_checkCatastrophicRm6 =
  verify checkCatastrophicRm "rm --recursive /etc/*$config*"
...
prop_checkCatastrophicRm11 =
  verify checkCatastrophicRm "rm -r /{bin,sbin}/$exec"

prop_checkCatastrophicRm12 =
  verify checkCatastrophicRm "rm -r /{{usr,},{bin,sbin}}/$exec"

prop_checkCatastrophicRm13 =
  verifyNot checkCatastrophicRm "rm -r /{{a,b},{c,d}}/$exec"

prop_checkCatastrophicRmA =
  verify checkCatastrophicRm "rm -rf /usr /lib/nvidia-current/xorg/xorg"

prop_checkCatastrophicRmB = verify checkCatastrophicRm "rm -rf \"$STEAMROOT/\"*"

checkCatastrophicRm =
  CommandCheck (Basename "rm") $ \t ->
    when (isRecursive t) $ mapM_ (mapM_ checkWord . braceExpand) $ arguments t
  where
      ...
      where
        ...
        f (T_DollarBraced _ _ word) =
          let var = onlyLiteralString word
           ...
           in if any (`isInfixOf` var) [":?", ":-", ":="]
                then Nothing
                else return ""
        f _ = return ""
    stripTrailing c = reverse . dropWhile (== c) . reverse
    skipRepeating c (a:b:rest)
      | a == b && b == c = skipRepeating c (b : rest)
...
prop_checkMvArguments6 =
  verifyNot checkMvArguments "mv --target-directory=foo bar"
...
prop_checkFindRedirections1 =
  verify checkFindRedirections "find . -exec echo {} > file \\;"

prop_checkFindRedirections2 =
  verifyNot checkFindRedirections "find . -exec echo {} \\; > file"

prop_checkFindRedirections3 =
  verifyNot checkFindRedirections "find . -execdir sh -c 'foo > file' \\;"
...
checkFindRedirections = CommandCheck (Basename "find") f
  where
    f t = do
      ...
      case redirecting of
        Just (T_Redirecting _ redirs@(_:_) (T_SimpleCommand _ _ args@(_:_:_)))
...
prop_checkSudoRedirect7 =
  verifyNot checkSudoRedirect "sudo cmd > /dev/null 2>&1"
...
checkSudoRedirect = CommandCheck (Basename "sudo") f
  where
    f t = do
      ...
      case t_redir of
        Just (T_Redirecting _ redirs _) -> mapM_ warnAbout redirs
  ...
  where
    f t =
      potentially $ do
        ...
        return $
          warn (getId t) 2232 $
          "Can't use sudo with builtins like " ++
          command ++ ". Did you want sudo sh -c .. instead?"
