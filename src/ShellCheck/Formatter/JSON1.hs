...
instance ToJSON Json1Output where
  toJSON result = object ["comments" .= comments result]
  toEncoding result = pairs ("comments" .= comments result)
    ...
    let start = pcStartPos comment
     ...
     in pairs
          ("file" .= posFile start <>
           ...
           "level" .= severityText comment <>
           "code" .= cCode c <>
           "message" .= cMessage c <> "fix" .= pcFix comment)

instance ToJSON Fix where
  toJSON fix = object ["replacements" .= fixReplacements fix]
