...
appendComments errRef comments max = do
  ...
  let current =
        map
          (\x -> (rankError x, cCode $ pcComment x, cMessage $ pcComment x))
          comments
...
outputForFile color sys comments = do
  ...
  mapM_
    (\commentsForLine -> do
       ...
       putStrLn ""
       putStrLn $
         color "message" $ "In " ++ fileName ++ " line " ++ show lineNum ++ ":"
       putStrLn (color "source" line)
       mapM_
         (\c -> putStrLn (color (severityText c) $ cuteIndent c))
         commentsForLine
       putStrLn ""
...
sliceFile fix lines = (mapPositions adjust fix, sliceLines lines)
  where
    (minLine, maxLine) =
      foldl
        (\(mm, mx) pos ->
  ...
  where
    ...
    makeArrow =
      let sameLine = lineNo comment == endLineNo comment
       ...
       in if sameLine && delta > 2 && delta < 32
            then arrow delta
            else "^--"
