...
data DiffDoc a =
  DiffDoc String LFStatus [DiffRegion a]
...
buildFixMap fixes = perFile
  where
    ...
    perFile =
      groupByMap (posFile . repStartPos . head . fixReplacements) splitFixes
...
prop_includesContextUntilEndIfNecessary =
  groupDiff [b 4, l 5] ==
      -- Nothing omitted
  [(True, [b 4, l 5])] -- Nothing Omitted

prop_splitsIntoMultipleHunks =
  groupDiff [l 1, b 1, b 2, b 3, b 4, b 5, b 6, b 7, r 8] ==
      -- Nothing omitted
  [(True, [l 1, b 1, b 2, b 3]), (False, [b 4]), (True, [b 5, b 6, b 7, r 8])] -- Nothing Omitted
