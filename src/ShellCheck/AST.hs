...
tokenEquals a b = kludge a == kludge b
  where
    kludge s = Re.subRegex (Re.mkRegex "\\(Id [0-9]+\\)") (show s) "(Id 0)"
...
analyze f g i = round
  where
    ...
    delve (T_DollarBraceCommandExpansion id list) =
      dl list $ T_DollarBraceCommandExpansion id
    ...
    delve (T_SourceCommand id includer t_include) =
      d2 includer t_include $ T_SourceCommand id
...
doStackAnalysis ::
     Monad m => (Token -> m ()) -> (Token -> m ()) -> Token -> m Token
