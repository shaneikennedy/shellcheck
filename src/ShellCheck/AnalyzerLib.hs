...
runChecker params checker = notes
  where
    ...
    check =
      perScript checker `composeAnalyzers`
      (\(Root x) -> void $ doAnalysis (perToken checker) x)
...
instance Monoid Checker where
  mempty = Checker {perScript = nullCheck, perToken = nullCheck}
...
makeComment severity id code note =
  newTokenComment
    { tcId = id
    , tcComment =
        newComment {cSeverity = severity, cCode = code, cMessage = note}
    }
...
warnWithFix ::
     MonadWriter [TokenComment] m => Id -> Code -> String -> Fix -> m ()
...
makeCommentWithFix severity id code str fix =
  let comment = makeComment severity id code str
      withFix = comment {tcFix = Just fix}
...
makeParameters spec =
  let params =
        Parameters
          { rootNode = root
          , shellType =
              fromMaybe (determineShell (asFallbackShell spec) root) $
              asShellType spec
          ...
          , hasLastpipe =
              case shellType params of
                ...
                Ksh -> True
          , shellTypeSpecified =
              isJust (asShellType spec) || isJust (asFallbackShell spec)
...
containsLastpipe root =
  ...
  where
    isShoptLastPipe t =
      case t of
        T_SimpleCommand {} ->
          t `isUnqualifiedCommand` "shopt" && ("lastpipe" `elem` oversimplify t)
        _ -> False
...
determineShellTest' fallbackShell =
  determineShell fallbackShell . fromJust . prRoot . pScript
...
determineShell fallbackShell t =
  ...
  where
    ...
    getCandidates (T_Annotation _ annotations s) =
      map forAnnotation annotations ++ [Just $ fromShebang s]
...
executableFromShebang = shellFor
  where
    shellFor s
      | "/env " `isInfixOf` s = head (drop 1 (words s) ++ [""])
    shellFor s
      | ' ' `elem` s = shellFor $ takeWhile (/= ' ') s
...
getTokenMap t = execState (doAnalysis f t) Map.empty
  where
...
isParamTo tree cmd = go
  where
    go x =
      case Map.lookup (getId x) tree of
        Nothing -> False
...
getClosestCommand tree t = findFirst findCommand $ getPath tree t
  where
    findCommand t =
      case t of
  ...
  where
    startScope t =
      let scopeType = leadType params t
       in do when (scopeType /= NoneScope) $ modify (StackScope scopeType :)
...
getModifiedVariables t =
  case t of
      ...
      concatMap
        (\x ->
           case x of
             ...
             _ -> [])
        vars
    c@T_SimpleCommand {} -> getModifiedVariableCommand c
    TA_Unary _ "++|" v@(TA_Variable _ name _) ->
      [(t, v, name, DataString $ SourceFrom [v])]
    TA_Unary _ "|++" v@(TA_Variable _ name _) ->
      [(t, v, name, DataString $ SourceFrom [v])]
    TA_Assignment _ op (TA_Variable _ name _) rhs ->
      maybeToList $ do
        guard $
          op `elem`
          ["=", "*=", "/=", "%=", "+=", "-=", "<<=", ">>=", "&=", "^=", "|="]
    ...
    TC_Unary id _ "-v" token ->
      maybeToList $ do
        str <-
          fmap (takeWhile (/= '[')) $ -- Quoted index
    ...
    t@(T_FdRedirect _ ('{':var) op) -- {foo}>&2 modifies foo
     ->
      [ (t, t, takeWhile (/= '}') var, DataString SourceInteger)
      | not $ isClosingFileOp op
      ]
    t@(T_CoProc _ name _) ->
...
getReferencedVariableCommand base@(T_SimpleCommand _ _ (T_NormalWord _ (T_Literal _ x:_):rest)) =
  case x of
    ...
    "trap" ->
      case rest of
        head:_ ->
          map (\x -> (head, head, x)) $ getVariablesFromLiteralToken head
        _ -> []
    _ -> []
  where
    getReference t@(T_Assignment _ _ name _ value) = [(t, t, name)]
    getReference t@(T_NormalWord _ [T_Literal _ name])
      | not ("-" `isPrefixOf` name) = [(t, t, name)]
    getReference _ = []
...
getModifiedVariableCommand base@(T_SimpleCommand _ _ (T_NormalWord _ (T_Literal _ x:_):rest)) =
  ...
  case x of
    "read" ->
      let params = map getLiteral rest
       ...
       in catMaybes . (++ readArrayVars) . takeWhile isJust . reverse $ params
    "getopts" ->
      case rest of
  ...
  where
    ...
    getModifierParam def t@T_NormalWord {} =
      maybeToList $ do
    ...
    letParamToLiteral token =
      if var == ""
        then []
               ...
               , token
               , var
               , DataString $ SourceFrom [stripEqualsFrom token])
             ]
      where
        var =
          takeWhile isVariableChar $
          dropWhile (`elem` "+-") $ concat $ oversimplify token
    getSetParams (t:_:rest)
      | getLiteralString t == Just "-o" = getSetParams rest
    getSetParams (t:rest) =
    ...
    getPrintfVariable list = f $ map (\x -> (x, getLiteralString x)) list
      where
        f ((_, Just "-v"):(t, Just var):_) =
          return (base, t, varName, varType $ SourceFrom list)
          where
            (varName, varType) =
              case elemIndex '[' var of
...
prop_getOffsetReferences4 =
  getOffsetReferences "[foo]:bar:baz" == ["bar", "baz"]

getOffsetReferences mods =
  fromMaybe [] $ do
...
getReferencedVariables parents t =
  ...
  where
    specialReferences name base word =
      if name `elem` ["PS1", "PS2", "PS3", "PS4", "PROMPT_COMMAND"]
        then map (\x -> (base, base, x)) $ getVariablesFromLiteralToken word
        else []
    literalizer t =
      case t of
        T_Glob _ s -> return s -- Also when parsed as globs
        _ -> Nothing
    getIfReference context token =
      maybeToList $ do
    ...
    isArithmeticAssignment t =
      case getPath parents t of
...
getVariablesFromLiteralToken token =
  getVariablesFromLiteral
    (fromJust $ getLiteralStringExt (const $ return " ") token)
...
getBracedReference s =
  ...
  where
    ...
    getSpecial (c:_) =
      if c `elem` "*@#?-$!"
        then return [c]
        else fail "not special"
    getSpecial _ = fail "empty"
    nameExpansion ('!':rest) -- e.g. ${!foo*bar*}
     = do
...
filterByAnnotation asSpec params = filter (not . shouldIgnore)
  where
...
shouldIgnoreCode params code t =
  any (isAnnotationIgnoringCode code) $ getPath (parentMap params) t
...
getOpts ::
     (Token -> [(Token, String)]) -> String -> Token -> Maybe [(String, Token)]
