...
module ShellCheck.Interface
  ( SystemInterface(..)
  , CheckSpec(csFilename, csScript, csCheckSourced, csIncludedWarnings,
          csExcludedWarnings, csShellTypeOverride, csMinSeverity, csIgnoreRC,
          csOptionalChecks)
  , CheckResult(crFilename, crComments)
  , ParseSpec(psFilename, psScript, psCheckSourced, psIgnoreRC,
          psShellTypeOverride)
  , ParseResult(prComments, prTokenPositions, prRoot)
  , AnalysisSpec(asScript, asShellType, asFallbackShell,
             asExecutionMode, asCheckSourced, asTokenPositions,
             asOptionalChecks)
  ...
  , Replacement(repStartPos, repEndPos, repString, repPrecedence,
            repInsertionPoint)
  , newReplacement
...
emptyCheckResult = CheckResult {crFilename = "", crComments = []}
...
newParseResult =
  ParseResult {prComments = [], prTokenPositions = Map.empty, prRoot = Nothing}
...
newAnalysisResult = AnalysisResult {arComments = []}
...
newFormatterOptions =
  FormatterOptions {foColorOption = ColorAuto, foWikiLinkCount = 3}
...
newCheckDescription =
  CheckDescription
    {cdName = "", cdDescription = "", cdPositive = "", cdNegative = ""}
...
newPosition = Position {posFile = "", posLine = 1, posColumn = 1}
...
newComment :: Comment
newComment = Comment {cSeverity = StyleC, cCode = 0, cMessage = ""}
...
newTokenComment =
  TokenComment {tcId = Id 0, tcComment = newComment, tcFix = Nothing}
...
mockedSystemInterface files =
  SystemInterface
    {siReadFile = rf, siFindSource = fs, siGetConfig = const $ return Nothing}
  where
    rf file =
