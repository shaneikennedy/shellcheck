...
getUnquotedLiteral (T_NormalWord _ list) = concat <$> mapM str list
  where
...
getTrailingUnquotedLiteral t =
  case t of
    (T_NormalWord _ list@(_:_)) -> from (last list)
    _ -> Nothing
  where
    from t =
      case t of
...
braceExpand (T_NormalWord id list) =
  take 1000 $ do
...
getCommandNameAndToken t =
  fromMaybe (Nothing, t) $ do
...
getAssociativeArrays t = nub . execWriter $ doAnalysis f t
  where
