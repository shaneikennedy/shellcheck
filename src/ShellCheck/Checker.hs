...
checkScript sys spec = do
  ...
  return emptyCheckResult {crFilename = csFilename spec, crComments = results}
  where
...
getErrors sys spec =
  sort . map getCode . crComments $ runIdentity (checkScript sys spec)
  where
    getCode = cCode . pcComment

check = checkWithIncludes []

checkWithSpec includes = getErrors (mockedSystemInterface includes)

checkWithIncludes includes src =
  checkWithSpec
    includes
    emptyCheckSpec {csScript = src, csExcludedWarnings = [2148]}

checkRecursive includes src =
  checkWithSpec
    includes
    emptyCheckSpec
      {csScript = src, csExcludedWarnings = [2148], csCheckSourced = True}

checkOptionIncludes includes src =
  checkWithSpec
    []
    emptyCheckSpec
      {csScript = src, csIncludedWarnings = includes, csCheckSourced = True}

checkWithRc rc = getErrors (mockRcFile rc $ mockedSystemInterface [])

checkWithIncludesAndSourcePath includes mapper =
  getErrors (mockedSystemInterface includes) {siFindSource = mapper}
...
prop_optionDisablesIssue1 =
  ...
  getErrors
    ...
    emptyCheckSpec {csScript = "echo $1", csExcludedWarnings = [2148, 2086]}
...
prop_optionDisablesIssue2 =
  ...
  getErrors
    (mockedSystemInterface [])
    emptyCheckSpec
      {csScript = "echo \"$10\"", csExcludedWarnings = [2148, 1037]}

prop_wontParseBadShell = [1071] == check "#!/usr/bin/python\ntrue $1\n"
...
prop_cantSourceDynamic = [1090] == checkWithIncludes [("lib", "")] ". \"$1\""
...
prop_deducesTypeFromExtension = null result
  where
    result =
      checkWithSpec
        []
        emptyCheckSpec {csFilename = "file.ksh", csScript = "(( 3.14 ))"}
...
prop_deducesTypeFromExtension2 = result == [2079]
  where
    result =
      checkWithSpec
        []
        emptyCheckSpec {csFilename = "file.bash", csScript = "(( 3.14 ))"}
...
prop_shExtensionDoesntMatter = result == [2148]
  where
    result =
      checkWithSpec
        []
        emptyCheckSpec {csFilename = "file.sh", csScript = "echo 'hello world'"}
...
prop_optionIncludes2
 ...
 =
  [2086] ==
  checkOptionIncludes (Just [2086]) "#!/bin/sh\n var='a b'\n echo $var"
...
prop_readsRcFile = null result
  where
    result =
      checkWithRc
        "disable=2086"
        emptyCheckSpec {csScript = "#!/bin/sh\necho $1", csIgnoreRC = False}
