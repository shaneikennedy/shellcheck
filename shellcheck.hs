...
defaultOptions =
  Options
    ...
    , sourcePaths = []
    , formatterOptions = newFormatterOptions {foColorOption = ColorAuto}
    , minSeverity = StyleC
    }
...
options =
  ...
  , Option "f" ["format"] (ReqArg (Flag "format") "FORMAT") $
...
runFormatter ::
     SystemInterface IO -> Formatter -> Options -> [FilePath] -> IO Status
  ...
  where
    ...
    process filename = do
      ...
      where
        check contents = do
          let checkspec =
                (checkSpec options) {csFilename = filename, csScript = contents}
...
parseOption flag options =
  case flag of
      ...
      fromMaybe (die $ "Unknown shell: " ++ str) $ do
        ...
        return $
          return
            options
              { checkSpec =
                  (checkSpec options) {csShellTypeOverride = Just shell}
              }
    Flag "exclude" str -> do
      ...
      return
        options
          {checkSpec = (checkSpec options) {csExcludedWarnings = new ++ old}}
    Flag "include" str -> do
      new <- mapM parseNum $ filter (not . null) $ split ',' str
    ...
    Flag "help" _ -> do
      ...
      throwError NoProblems
    Flag "externals" _ -> return options {externalSources = True}
    Flag "color" color -> do
      option <- parseColorOption color
      return
        options
          { formatterOptions =
              (formatterOptions options) {foColorOption = option}
          }
    ...
    Flag "severity" severity -> do
      ...
      return options {checkSpec = (checkSpec options) {csMinSeverity = option}}
    Flag "wiki-link-count" countString -> do
      count <- parseNum countString
      return
        options
          { formatterOptions =
              (formatterOptions options) {foWikiLinkCount = count}
          }
    Flag "norc" _ ->
      return options {checkSpec = (checkSpec options) {csIgnoreRC = True}}
    Flag "enable" value ->
...
ioInterface options files = do
  ...
  where
    ...
    fetch cache inputs file = do
      ...
      if ok
        then (do (contents, shouldCache) <- inputFile file
                 when shouldCache $ modifyIORef cache $ Map.insert file contents
                 return $ Right contents) `catch`
             handler
        else return $
             Left (file ++ " was not specified as input (see shellcheck -x).")
      where
    ...
    normalize x = canonicalizePath x `catch` fallback x
      where
    ...
    findSourceFile inputs sourcePathFlag currentScript sourcePathAnnotation original =
      if isAbsolute original
        then let (_, relative) = splitDrive original
...
inputFile file = do
  ...
  seq (length contents) $ return (contents, shouldCache)
